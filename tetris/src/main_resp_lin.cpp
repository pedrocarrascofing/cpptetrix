...
int main(int argc, char* argv[])
{
	...
	string dir_imagenes, mens_titulo, mens_respiros, aux;;
	int semilla, t_inicial_espera, line_nivel, n_piezas_cola, n_imagenes, n_piezas, pieza, n_respiros;
	int n_lineas_activacion, n_lineas_sig;
	Vis_txt v_titulo, v_nivel, v_lineas, v_marcador, v_estado, v_respiros;
...
////////Parámetros relacionados con la dinámica del juego//////////////
	...
	if(n_respiros<0)
	{
		cerr << "Problema con el numero de respiros " << endl;
		return -1;
	}
	...
	IgnorarComentarios(f);
	f>>n_lineas_activacion;

	//Cada cuántas líneas terminadas se introduce una nueva
	IgnorarComentarios(f);
	f>>n_lineas_sig;
	if(n_lineas_sig < 0  || n_lineas_activacion < 0)
	{
		cerr << "Problema con la introduccion de lineas " << endl;
		return -1;
	}
...
////////Creacion de la ventana y de los visualizadores ////////////////
	...
	v_respiros.RefreshVis();
	v_respiros.RefreshTxt(mens_respiros);
	v_estado.RefreshVis();
...
////////Desarrollo del juego//////////////////
	...
	int n_aux, cont_lineas=0, cont_piezas=1, aux_l=0, t_espera=t_inicial_espera, nivel, nivel_anterior=0, cont_l_aleat=n_lineas_activacion;
	bool GameOver=false, posible_abajo=true, posible_arriba=false, fin_aleat=true;
	Color c_aux;
	...
	do{
		...
		switch(tec)
		{
			case SDLK_UP:	posible_arriba=MoverArriba(tablero, visual_tab, n_respiros);
					break;
		...
		if(posible_arriba)
		{
			...
			conver.clear();
			v_respiros.RefreshVis();
			v_respiros.RefreshTxt("Respiros " + aux);	
			posible_arriba=false;
		}


		if(cont_lineas%n_lineas_sig==0 && n_aux!= cont_lineas || cont_l_aleat<=0)
			fin_aleat=false;
		

		if(!fin_aleat)
			if(cont_lineas>=n_lineas_activacion)	
			{
				cont_l_aleat=n_lineas_sig;
				n_aux=cont_lineas;
				fin_aleat=true;
				tablero.LineaAleatoria(n_imagenes);
			}
